...
object FormIV {
    ...
    fun generate(r1: Char, r2: Char, r3: Char): List<ConjugationForm> {
        ...
        val presPrefixes = listOf(
            PrefixInfo("يُ", "", "3","m","s"), PrefixInfo("تُ", "", "3","f","s"),
            PrefixInfo("يُ", "َانِ", "3","m","d"), PrefixInfo("تُ", "َانِ", "3","f","d"),
            PrefixInfo("يُ", "ُونَ", "3","m","p"), PrefixInfo("يُ", "نَ", "3","f","p"),
            PrefixInfo("تُ", "", "2","m","s"), PrefixInfo("تُ", "ِينَ", "2","f","s"),
            PrefixInfo("تُ", "َانِ", "2","","d"), PrefixInfo("تُ", "ُونَ", "2","m","p"),
            PrefixInfo("تُ", "نَ", "2","f","p"), PrefixInfo("أُ", "", "1","","s"),
            PrefixInfo("نُ", "", "1","","p")
        )

        val pastActiveStem = "أَ${r1}ْ${r2}َ${r3}"
        val pastPassiveStem = "أُ${r1}ْ${r2}ِ${r3}"
        val presActiveStem = "${r1}ْ${r2}ِ${r3}"
        val presPassiveStem = "${r1}ْ${r2}َ${r3}"

        generatePastForms(forms, pastSuffixes, pastActiveStem, pastPassiveStem, rootStr)
        generatePresentForms(forms, presPrefixes, presActiveStem, presPassiveStem, rootStr)
